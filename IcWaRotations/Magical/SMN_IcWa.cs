...
[Rotation("IcWa's Summoner", CombatType.PvE, GameVersion = "7.0")]
[SourceCode(Path = "main/DefaultRotations/Magical/SMN_IcWa.cs")]
[Api(2)]
public sealed class SMN_IcWa : SummonerRotation
{
    ...
    protected override bool AttackAbility(IAction nextGCD, out IAction? act)
    {
        ...
        if (IsBurst && InBahamut || InSolarBahamut && UseMedicine && !Player.HasStatus(false, StatusID.SearingLight) && SummonBahamutPvE.Cooldown.ElapsedOneChargeAfterGCD(0) || SummonSolarBahamutPvE.Cooldown.ElapsedOneChargeAfterGCD(0) )
        {
            if (UseBurstMedicine(out act)) return true;
        }
        
        if (IsBurst && !Player.HasStatus(false, StatusID.SearingLight) && InBahamut || InSolarBahamut && SummonBahamutPvE.Cooldown.ElapsedOneChargeAfterGCD(1) || SummonSolarBahamutPvE.Cooldown.ElapsedOneChargeAfterGCD(1) )
        {
    ...
    protected override bool GeneralGCD(out IAction? act)
    {
        ...
        if (!SearingLightPvE.Cooldown.IsCoolingDown && SummonBahamutPvE.CanUse(out act)) return true;
